...
% earlier in the term
% -----------------------
% just extract the shallowest 50m of depth and temperature arrays because 
% there are measurements at these depths on all tracks
tempr=tempr(1:51,:);       % <-- lab modification needed - change to saln
depths=depths(1:51);
...
for j=10:-1:1
    ...
    ii=inds{j};
    subtemp=tempr(:,ii);     % <-- lab modification needed, change to saln  
    xx=dist200(ii);
    ...
    makeScatterPlot(mx(:),my(:),subtemp(:),j,k);
    colorbar;
    caxis([6 15])      %  <-- lab modification needed, uncomment and then reset if desired to salinity values [32 34.2] ish
    xlim([-20 45]);  
...
% need to modify this to save salinity
save lab11_data.mat lon lat dist200 depths tempr inds    % <-- lab modification needed - save saln
