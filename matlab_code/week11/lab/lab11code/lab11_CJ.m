...
for j=1:10
    ...
    figure(2);
    jj=isfinite(intSalr(:));  %ignore nans because scatter colors nans  or use ~isnan
    ...
    % check have USED LINEAR INTERPOLATION FOR LAST PART OF LAB
...
hold on;
    ...
    stdval = nanstd(sal0_pro, 0, 2);   %std along second dimension - note matlab requires the flag=0,1 before dimension
    h1=plot(meanval, depths, 'k', 'linewidth', 2); %one col of zz
    plot(meanval+stdval, zz, '-k');
    plot(meanval-stdval, zz, '-k');

    meanval=nanmean(sal20_pro, 2);   %need mean of second dimension 
    stdval20 = nanstd(sal20_pro')';  %std along 2nd dimension w/o flag but have to transpose and then again for plotting
    h2=plot(meanval, depths, '-r', 'linewidth', 2); %one col of zz
    plot(meanval+stdval20, zz, '--r');
    plot(meanval-stdval20, zz, '--r');

    legend([h1, h2],'0 km \pm 1\sigma','20 km \pm 1\sigma');
    set(gca,'FontSize',14);
    xlabel('Salinity/(g/kg)');
    title('c)');
    ylabel('Depth (m)');
...
figure(3);print -depsc finalfig_2018
